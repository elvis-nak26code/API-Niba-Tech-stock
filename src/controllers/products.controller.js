...
export async function enrichProducts(products) {
  ...
  const [cats, sups] = await Promise.all([
    ...
    supIds.length ? Supplier.find({ _id: { $in: supIds } }).lean() : [],
  ])
...
export const list = catchAsync(async (req, res) => {
  const products = await Product.find({ isDeleted: { $ne: true } }).sort({ name: 1 }).lean()
  ok(res, await enrichProducts(products))
})
...
export const get = catchAsync(async (req, res) => {
  ...
  ok(res, (await enrichProducts([product]))[0])
})
...
export const create = catchAsync(async (req, res) => {
  ...
  ok(res, (await enrichProducts([product.toObject()]))[0], 201)
})
...
export const update = catchAsync(async (req, res) => {
  ...
  if (sku && sku !== product.sku) {
    const dup = await Product.findOne({ sku, _id: { $ne: product._id }, isDeleted: { $ne: true } })
  ...
  if (categoryId !== undefined) {
    const category = categoryId ? await Category.findById(categoryId).lean() : null
  ...
  await recomputeAlerts()
  ok(res, (await enrichProducts([product.toObject()]))[0])
})
